 ...
 * NestedSetSaveAction
 * 
 * Действие сохранения элеметов модели с NestedSetBehavior
 *
 */
...
use YiiHelper as Y;
use ARHelper as HAR;
use AjaxHelper as HAjax;

class SaveAction extends \CAction
{
	/**
	 * @var string имя класса модели \CActiveRecord с поведение \NestedSetBehavior. 
	 */
	public $modelClass;
	...
	/**
	 * @var string имя поведения \NestedSetBehavior в модели.
	 */
	...
	public function run()
	{
		...
		$data=json_decode(Y::request()->getPost($this->paramData, json_encode([])));
		if(is_array($data)) {
			$cases=[
			...
			$query='UPDATE '.HAR::dbQT($model->tableName()).' AS `t` SET '.implode(',', $cases);
