...
class HSettings 
{
	 ...
	 */
	public static function getMenuItems($controller, $id=null, $baseUrl='settings/admin/default/index')
	{
		$items=[];
		
		$module=Y::module('settings')->getModule('admin');
		...
		if($config=self::config()) {
			...
			$fAddItem=function($id, $params) use (&$items, $controller, $baseUrl, $config) {
				if($label=A::get($params, 'menuItemLabel', A::get($params, 'title'))) {
					$items[]=[
						'label'=>$label,
						'url'=>$controller->createUrl($baseUrl, ['id'=>$id])
					];
				}
			};
